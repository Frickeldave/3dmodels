...
difference() {
    ...
    translate([-125, -145, 20])
    union() {
        // DJI Gimbal SVG - mittig platziert
        
        linear_extrude(height = 80)
        scale([1.0, 1.0, 1.0])
        import("dji-gimbal.svg");

        color("red")
        hull() {

            translate([85, 100, 35])
            cylinder(h=80, r=12, center=true);


            translate([150, 100, 35])
            cylinder(h=80, r=12, center=true);
        }
    }
    ...
    // DJI Gimbal mobile holder SVG - mittig platziert
    translate([-65, -200, 10])
    linear_extrude(height = 80)
    scale([1.0, 1.0, 1.0])
    import("dji-gimbal-mobile-holder.svg");
